...
export const applyFunctionSchema = {
  definition: z
    .string()
    .describe('Definição completa da função a aplicar: CREATE OR REPLACE FUNCTION ...'),
};
...
export async function applyFunctionHandler(args: { definition: string }) {
  ...
  if (!ident) {
    return {
      content: [
        {
          type: 'text' as const,
          text: 'Function applied successfully (could not parse identifier to fetch back).',
        },
      ],
    };
  }
